...
namespace Chess
{
    public class Movement {
        ...
        public bool IsKingSafe(){
            ...
            for (int i = 0; i < availableMoves.Length; i++)
                if (availableMoves[i].HasValue)
                    return true;//это означает, что есть решение, позволяющее предотвратить мат.
            return false;
        }
        ...
        PieceMove?[] Moves() {
            ...
            switch (pieceTile.piece.piecekind) {
                ...
                case PieceKind.King:
                    ...
                    if (pieceTile.piece.firstMove && Board.GetPiece(pieceTile.GetY, pieceTile.GetX + 3).firstMove) //правильная рокировка
                        moves[8] = CanCastle(1) ? destination(0,2) : null;
                    if (pieceTile.piece.firstMove && Board.GetPiece(pieceTile.GetY, pieceTile.GetX - 4).firstMove) //левая рокировка
        ...
        void LineMove(PieceMove?[] moves,bool straightLine){ // для таких фигур, как Слон, Ферзь и Ладья
        ...
        PieceMove? destination(int y, int x) { // для таких фигур, как ферзь и король
